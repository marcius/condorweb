 ...
 * @version 1.0
 */
class ArrayDataProviderEx2 extends CDataProvider {
    ...
    public function __construct($array, $arrayType = self::ARRAY_ASSOC){
        if(($arrayType != self::ARRAY_ASSOC) && ($arrayType != self::ARRAY_ORDER) && ($arrayType != self::ARRAY_EMPTY)){
            throw new CException('Unknown array type'); // use Yii::t
        }

        if ($arrayType == self::ARRAY_EMPTY) {
            return;
        }
